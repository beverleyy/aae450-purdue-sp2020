% Play around with angle, Ballistic Coefficient
%lower angles break
Vinf = 5*10^3; % m/s
...
% figure()
% plot(y3(:,7)/1000,(y3(:,1)- 3389.5*10^3)/1000)
% hold on;
% plot(y4(:,7)/1000,(y4(:,1)- 3389.5*10^3)/1000)
% legend('bank angle 0','bank angle 180');
% grid on;
...
CD = 1.7;%coeff drag
A = 50;%Ref Area approx of spac9e shuttle lander
LD = 0; % 0.8 % Lift/Drag approximated from space shuttle
Beta = 300; %m / (CD*A)%Ballistic Coefficient
mu_p = 4.282837*10^13; %m^3/s^2 grav param mars
...
%was gr*cos(5) now is gr*cos(y(5)), and is not working
dydt(5)= (q*LD)/(y(4)*Beta)*cos(sigma) + 1/y(4)*(gr*cos(y(5)) + (y(4)*cos(y(5)))/y(1));
